...
export default (inputData, stretchFactor) => {
  const leftChannel = new Float32Array(inputData.length / 2);
  const rightChannel = new Float32Array(inputData.length / 2);
  for (let i = 0; i < inputData.length; i += 1) {
    if (i % 2 === 0) {
      leftChannel[i >>> 1] = inputData[i];
    } else {
      rightChannel[i >>> 1] = inputData[i];
    }
  }
